...
export default function LiveAssessmentDashboard() {
  ...
  const currentAthleteObj = athletes.find((a) => a.id === selectedAthleteId);
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2 border-t border-slate-200 dark:border-slate-800 text-xs">
          <div>
            ...
            >
              <option value="all">All Athletes (Consolidated)</option>
              {athletes.map((ath) => (
      ...
      {isPermissionDenied && (
        <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-300 text-xs space-y-1">
          ...
          <p className="text-[11px] text-amber-800 dark:text-amber-400">
            Assessment access requires a valid authenticated role. No fallback data is displayed.
          </p>
        </div>
      )}
      ...
      {error && !isPermissionDenied && (
        <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-800 dark:text-rose-300 text-xs">
          Unable to load assessments: {error}
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-bold text-slate-900 dark:text-white">
              Live Session Assessment Feed (Postgres: assessments)
            </h4>
          </div>
